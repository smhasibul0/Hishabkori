...
<div class="content">
    <div class="container-xxl">
        ...
        <style>
            ...
            #supplier-table thead th:last-child { border-right: none; }
            #supplier-table thead th:hover { background: #f1f5f9; color: #1e293b; }
            #supplier-table thead th.sort-asc::after { content: ' ↑'; color: #6366f1; }
            #supplier-table thead th.sort-desc::after { content: ' ↓'; color: #6366f1; }

            #supplier-table tbody tr { border-bottom: 1px solid #e2e8f0; transition: background .1s; }
            #supplier-table tbody tr:nth-child(even) { background: #f8fafc; }
            #supplier-table tbody tr:hover { background: #eef2ff; }
            ...
            #supplier-table tbody td:last-child { border-right: none; }

            #supplier-table tfoot td {
                background: #f1f5f9;
                font-weight: 700;
                color: #1e293b;
                padding: .55rem .9rem;
                                ...
                                <table id="supplier-table">
                                    <thead>
                                        <tr>
                                            ...
                                            <th data-col="2">Business Name</th>
                                            <th data-col="3">Name</th>
                                            <th data-col="4">Email</th>
                                            <th data-col="5">Phone</th>
                                            <th data-col="6">Pay Term</th>
                                            <th data-col="7">Opening Balance</th>
                                            <th data-col="8">Advance Balance</th>
                                            <th data-col="9">Added On</th>
                                            <th data-col="10">Address</th>
                                            <th data-col="12">Total Purchase Due</th>
                                            <th data-col="13">Total Purchase Return Due</th>
                                            <th data-col="14">Tax Number</th>
                                        </tr>
                                    </thead>
                                    <tbody id="tableBody">
                                        ...
                                        <tr>
                                            <td>
                                                ...
                                                <div class="dropdown-menu">
                                                    ...
                                                    <a class="dropdown-item" href="{{ route('edit.supplier', $item->id) }}">Edit</a>
                                                    <a href="{{ route('delete.supplier', $item->id) }}" class="dropdown-item" id="delete">Delete</a>
                                            ...
                                            <td>৳ {{ number_format($item->purchase_due, 2) }}</td>
                                            <td>৳ {{ number_format($item->purchase_return_due, 2) }}</td>
                                    ...
                                    <tfoot id="tableFoot">
                                        <tr>
                                            <td colspan="12" class="text-end pe-3">Total:</td>
                                            <td>৳ {{ number_format($suppliers->sum('purchase_due'), 2) }}</td>
                                            <td>৳ {{ number_format($suppliers->sum('purchase_return_due'), 2) }}</td>
                                            <td></td>
                                        </tr>
                                    </tfoot>
                                </table>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // ── collect all rows from the rendered HTML ──
    ...
    function render() {
        ...
        if (filtered.length === 0) {
            if (!document.getElementById('no-results-row')) {
                ...
                tr.innerHTML = `<td colspan="15" class="no-results">No matching records found</td>`;
    ...
    document.getElementById('searchInput').addEventListener('input', function () {
        ...
        filtered = allRows.filter(row =>
            Array.from(row.querySelectorAll('td'))
                 ...
                 .some(td => td.innerText.toLowerCase().includes(q))
        );
        currentPage = 1;
        render();
    });
    ...
    // ── column sort ──
    document.querySelectorAll('#supplier-table thead th').forEach(th => {
        ...
        th.addEventListener('click', () => {
            ...
            document.querySelectorAll('#supplier-table thead th')
