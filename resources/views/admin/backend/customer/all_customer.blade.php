...
<div class="content">
    <div class="container-xxl">
        ...
        <style>
            ...
            .dt-controls-left select:focus { border-color:#6366f1; }
            ...
            .dt-scroll-area::-webkit-scrollbar-track { background:transparent; }
            ...
            #customer-table { width:100%; min-width:1400px; border-collapse:collapse; font-size:.8rem; margin-bottom:0; }
            #customer-table thead th { background:#f8fafc; color:#475569; font-size:.72rem; font-weight:700; text-transform:uppercase; letter-spacing:.05em; padding:.6rem .9rem; white-space:nowrap; border-bottom:2px solid #e2e8f0; border-right:1px solid #e2e8f0; position:sticky; top:0; z-index:2; cursor:pointer; user-select:none; }
            #customer-table thead th:last-child { border-right:none; }
            #customer-table thead th:hover { background:#f1f5f9; color:#1e293b; }
            #customer-table thead th.sort-asc::after { content:' ↑'; color:#6366f1; }
            #customer-table thead th.sort-desc::after { content:' ↓'; color:#6366f1; }
            #customer-table tbody tr { border-bottom:1px solid #e2e8f0; transition:background .1s; }
            #customer-table tbody tr:nth-child(even) { background:#f8fafc; }
            #customer-table tbody tr:hover { background:#eef2ff; }
            #customer-table tbody td { padding:.55rem .9rem; white-space:nowrap; color:#334155; border-right:1px solid #e2e8f0; }
            #customer-table tbody td:last-child { border-right:none; }
            #customer-table tfoot td { background:#f1f5f9; font-weight:700; color:#1e293b; padding:.55rem .9rem; border-top:2px solid #e2e8f0; white-space:nowrap; }
                                ...
                                <table id="customer-table">
                                    <thead>
                                        <tr>
                                            ...
                                            <th data-col="1">Contact Id</th>
                                            <th data-col="3">Name</th>
                                            <th data-col="4">Email</th>
                                            <th data-col="5">Phone</th>
                                            <th data-col="6">Customer Group</th>
                                            <th data-col="7">Pay Term</th>
                                            <th data-col="8">Opening Balance</th>
                                            <th data-col="9">Advance Balance</th>
                                            <th data-col="10">Address</th>
                                            <th data-col="11">Sale Due</th>
                                            <th data-col="12">Sale Return Due</th>
                                            <th data-col="13">Tax Number</th>
                                        </tr>
                                    </thead>
                                    <tbody id="tableBody">
                                        ...
                                        <tr>
                                            <td>
                                                ...
                                                <div class="dropdown-menu">
                                                    ...
                                                    <a class="dropdown-item" href="{{ route('edit.customer', $item->id) }}">Edit</a>
                                            ...
                                            <td>{{ $item->pay_term }}</td>
                                            <td>৳ {{ number_format($item->openning_balance, 2) }}</td>
                                            <td>৳ {{ number_format($item->advance_balance, 2) }}</td>
                                            <td>{{ Str::limit($item->address, 50, '...') }}</td>
                                            <td>৳ {{ number_format($item->sale_due, 2) }}</td>
                                            <td>৳ {{ number_format($item->sale_return_due, 2) }}</td>
                                    ...
                                    <tfoot id="tableFoot">
                                        <tr>
                                            <td colspan="11" class="text-end pe-3">Total:</td>
                                            <td>৳ {{ number_format($customers->sum('sale_due'), 2) }}</td>
                                            <td>৳ {{ number_format($customers->sum('sale_return_due'), 2) }}</td>
                                            <td></td>
                                        </tr>
                                    </tfoot>
                                </table>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function render() {
        ...
        if (filtered.length === 0) {
            if (!document.getElementById('no-results-row')) {
                ...
                tr.innerHTML = `<td colspan="14" class="no-results">No matching records found</td>`;
    ...
    document.querySelectorAll('#customer-table thead th').forEach(th => {
        ...
        th.addEventListener('click', () => {
            ...
            sortCol   = col;
            document.querySelectorAll('#customer-table thead th').forEach(h => h.classList.remove('sort-asc', 'sort-desc'));
